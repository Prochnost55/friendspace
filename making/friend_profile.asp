...
name1 = server.HTMLEncode(Cstr(name1))
					name1test= name1
					length = len(name1test)
					name1 = ucase(left(name1,1))
					name1test = lcase(mid(name1test,2)) 
					name1 = name1 & name1test
...
Flag=0
Do until objRS.EOF
	If Lcase(Trim(objRS("name")))= lcase(Trim(name1)) Then
...
<div id="container">
 <div id="header"> <img src='<%response.Write(user&".jpg")%>' alt="b.jpg" name="photo" border="2" id="photo"> <span id="header">Welcome, <%=response.write(username & " " & lastname) %> </span>
...
<%
			...
			Set Myfile=Fileobj.openTextFile("C:\data\"&Ruser&"\Profile.txt",8,1 )
